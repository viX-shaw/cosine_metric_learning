...
def read_image_directory_to_str(directory, data_dir):
    ...
    with open(os.path.join(data_dir, "vric_train.txt"), 'r') as f:
        ...
        for entry in sorted(lines):
            filename, _id = [x for idx, x in enumerate(entry.split()) if idx == 0 or idx == 2]
            image_filenames.append(os.path.join(directory, filename))
            ids.append(int(_id))
    return image_filenames, ids


def read_image_directory_to_image(directory, data_dir):
...
def read_train_split_to_str(dataset_dir):
    ...
    """
    image_directory = os.path.join(dataset_dir, "train_images")
...
def read_train_split_to_image(dataset_dir):
    ...
    """
    image_directory = os.path.join(dataset_dir, "train_images")
    return read_image_directory_to_image(image_directory, dataset_dir)
